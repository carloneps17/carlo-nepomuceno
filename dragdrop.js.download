...
dragdrop.ModuleReordering.prototype =
{
    ...
    if ( this.moveAccessibleControlsToActionBar )
    {
      ...
      if ( actionBar )
      {
        ...
        if ( actionBarButtons )
        {
          ...
          if ( firstSecondaryButton )
          {
            firstSecondaryButton.after( newButton );
          }
          else
          {
            actionBarButtons.appendChild( newButton );
  ...
  onShowReorderingControlsClick: function( event )
  {
    ...
    {
      offset[0] = offset[0] - width + 30;
      //When zoom is set to 250%, the reorder popup div.keyboardAccess will be set to 100% width.
      //So the offset[0] value goes to negative value. like , -678px. Therefore popup is only partially displayed in the UI.
      //Hence the below lines are added to make sure the popup is completely visible in 250% zoom.
      if( offset[0] < 0 )
      {
        offset[0] = 3;
      }
